      ...
      >
        <div class="typeology-result-head">
          ...
          <p class="typeology-result-score">
            {{ currentResult.scoreLabel || "匹配度" }}：{{ currentResult.mainResult.score }}% ·
...
/**
 * 当前页面对应的授权结果同步目标路径。
 * 关键逻辑：`/mbti` 与 `/shmbti` 虽然 UI 不同，但都要基于当前授权上下文隔离结果缓存。
 */
...
const insightForView = computed(() => {
  const streamingShortSummary = String(
    aiStreamingShortSummaryText.value ?? "",
  ).trim();
  if (isAiInsightStreaming.value && streamingShortSummary) {
    return streamingShortSummary;
  }

  const aiShortSummary = String(
    currentResult.value?.aiInsight?.shortSummary ?? "",
  ).trim();
  if (aiShortSummary) {
  ...
  const summaryOverride = String(
    currentResult.value?.summaryText ?? "",
  ).trim();
  if (summaryOverride) {
...
.typeology-dual-high-chip {
  border-radius: 999px;
  border: 1px solid
    color-mix(in srgb, var(--type-accent) 26%, var(--type-card-border) 74%);
  background: color-mix(
    in srgb,
    var(--type-accent) 8%,
    var(--type-card-bg) 92%
