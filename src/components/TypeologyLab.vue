    ...
    <main class="typeology-shell" aria-live="polite">
      ...
      <section class="typeology-panel typeology-knowledge-panel">
        ...
        <article class="typeology-knowledge-item">
          ...
          >
            {{ displayedResultCache[activeTestConfig.key] ? "查看该测试结果" : "进入该测试" }}
          </button>
        </article>
...
/**
 * 生成 AI 流式短摘要预览文本。
 * 关键逻辑：对流式 `shortSummary` 也执行同一套脏词拦截，避免提示模板在主卡闪现。
 * 复杂度评估：O(L)，L 为当前累计文本长度。
 * @param {string} streamRawText 流式累计文本。
...
function resolveAiNarrativePreviewText(streamRawText) {
  const narrativePreview = sanitizeAiCopyText({
    text: extractFieldPreviewFromJsonStream(streamRawText, "narrative"),
    fallbackText: "",
  });
  if (narrativePreview) {
