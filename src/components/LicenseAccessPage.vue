  ...
  <section class="license-auth-shell">
    <div class="license-auth-panel">
      ...
      <div class="license-auth-form">
        <label class="license-auth-label" for="license-code-input">授权码</label>
        <input
      ...
      <p class="license-auth-tip">
        授权成功后将绑定当前浏览器环境；更换浏览器、清除本地缓存或更换设备后，系统会要求重新校验并可能拒绝访问。
      </p>
    </div>
  </section>
</template>
...
function resolveErrorMessage(errorCode) {
  const errorMessageMap = {
    LICENSE_API_BASE_URL_MISSING: "未配置授权服务地址，请先补充 VITE_LICENSE_API_BASE_URL。",
    LICENSE_CODE_NOT_FOUND: "授权码不存在，请检查后重试。",
    LICENSE_CODE_DISABLED: "该授权码已停用，请联系管理员。",
    LICENSE_DEVICE_MISMATCH: "该授权码已绑定其他设备，当前设备无法使用。",
    ...
    SCOPE_NOT_GRANTED: "该授权码尚未获得当前页面的访问权限，请输入授权码继续授权。",
    DEVICE_MISMATCH: "当前浏览器环境与已绑定记录不一致，请使用原设备访问。",
    SESSION_MISSING: "",
  };

  return errorMessageMap[errorCode] || "授权失败，请稍后重试。";
}
...
.license-auth-shell {
  ...
  background:
    radial-gradient(circle at top left, rgba(128, 90, 213, 0.24), transparent 42%),
    radial-gradient(circle at bottom right, rgba(94, 85, 116, 0.28), transparent 40%),
